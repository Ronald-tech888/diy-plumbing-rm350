window.renderBookingSummary = function(node,s,t,zh){
  ...
  for(const [label,value] of [[zh?'日期':'Date',date],[zh?'授课语言':'Language',s?s.language:(zh?'—':'—')],[zh?'时间（马来西亚）':'Time (Malaysia)',s?s.time:'—'],[zh?'票种与人数':'Ticket & participants',ticket+(t?` · ${t.seats}${zh?'人':' pax'}`:'')]]){
